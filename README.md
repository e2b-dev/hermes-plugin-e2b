...
- a file both sides have with different contents is a genuine conflict, since no baseline says which is newer. The host copy is left alone and the sandbox copy is saved under `~/.hermes/cache/e2b-recovered/<sandbox id>/`, named in a warning. Neither version is discarded. So is a sandbox file whose host path holds something other than a regular file, and one whose host copy cannot be read;
